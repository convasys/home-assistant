...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback):
    ...
    def on_message(client, userdata, msg):
        try:
            ...
            _LOGGER.info("🧩 Discovered Lytiva scene: %s (%s)", name, unique_id)
            scene_entity = LytivaScene(name, unique_id, command_topic, state_topic, payload_on, mqtt)
...
class LytivaScene(Scene):
    ...
    def __init__(self, name, unique_id, command_topic, state_topic, payload_on, mqtt_client):
        self._name = name
